...
Web platform for discovering, visiting, and tracking sacred sites.
Platforms: desktop web, mobile web, and admin web.
...
No shared `packages/` folder. Customer web and admin web own their API clients and types.
...
All customer-facing UI strings come from `GET /api/v1/translations?lang=`. Supported languages: English (default), Arabic, Hindi, Telugu, Malayalam. RTL layout enabled automatically when locale is `ar`. Never hardcode copy in frontend files.
...
### Web-only customer app

The customer product is a responsive Next.js web app. Mobile web uses the same routes and components as desktop web, with layout-specific bottom-navigation offsets.
...
**Portable run handoff:** runs can now be leased into a `RunHandoff`, exported into a portable bundle, resumed locally against a snapshot DB, and finalized back into production as the same `run_code`. Finalize uploads are streamed as gzip bundles through the scraper nginx vhost, whose request-body limit is higher than the catalog vhost because completed country runs can exceed 20 MB after compression. For bundles too large for the VM scraper API's memory envelope, the operator CLI can finalize directly from the local handoff DB into production scraper Postgres in chunks, then trigger the same direct catalog sync flow. While a handoff is active, mutating run actions are blocked until the handoff is finalized or aborted.
