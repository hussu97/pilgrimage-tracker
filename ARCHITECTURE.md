...
Place attributes (capacity, parking, wheelchair access, etc.) are stored using an **Entity-Attribute-Value (EAV)** pattern for flexibility:

- **PlaceAttributeDefinition** defines available attributes (e.g., `has_parking`, `capacity`, `denomination`). Each definition includes metadata: data type, icon, i18n label key, whether it's filterable or a specification, religion-specific constraints, and display order.
...
- **Benefits:** Adding new scraped fields or religion-specific attributes requires only a DB row (not code changes). Specifications and filter chips are dynamically generated from attribute definitions.
- **Backward compatibility:** The `religion_specific` JSON column remains for legacy data; new code reads from PlaceAttribute first, falls back to `religion_specific`.
