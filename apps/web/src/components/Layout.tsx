...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col font-display">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-white/50 z-40 pb-[env(safe-area-inset-bottom,20px)] pt-2 px-2" aria-label="Main navigation">
        <div className="grid grid-cols-4 gap-1 max-w-md mx-auto">
          {navItems.map(({ path, labelKey, icon }) => {
            ...
            const showDot = isActive && icon === 'explore' && unreadCount > 0;
            return (
              <Link
                key={path}
                to={path}
                aria-current={isActive ? 'page' : undefined}
                className={`flex flex-col items-center justify-center gap-1.5 pt-2 rounded-xl transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 ${
                  isActive ? 'text-primary' : 'text-slate-400 hover:text-slate-600'
                }`}
              >
                <div className="relative p-1">
                  <span
                    className={`material-symbols-outlined text-[26px] ${isActive ? 'drop-shadow-sm' : 'font-light'}`}
                  ...
                  {showDot && (
                    <span className="absolute top-1 right-0 w-1.5 h-1.5 bg-red-400 rounded-full ring-2 ring-white" />
                  )}
                </div>
