...
export default function FilterSheet({
    ...
    const handleClear = () => {
        const cleared = Object.keys(pending).reduce((acc, key) => {
            acc[key] = false;
            return acc;
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${isActive ? 'bg-primary text-white' : 'bg-slate-100 text-slate-500'
                                            }`}>
                                            <span className="material-symbols-outlined text-[20px]">{option.icon}</span>
                                        </div>
                                        <div className="text-left">
                                            <span className={`block font-semibold ${isActive ? 'text-primary' : 'text-text-main'}`}>
                                                {option.label}
                                            </span>
                                    ...
                                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${isActive ? 'border-primary bg-primary' : 'border-slate-200'
                                        }`}>
                                        {isActive && (
