...
export default function ResetPassword() {
  ...
  if (success) {
    return (
      <div className="max-w-md mx-auto px-6 py-8 safe-area-top safe-area-bottom">
        <h1 className="text-2xl font-bold text-text-main mb-2">{t('auth.passwordUpdated')}</h1>
        <p className="text-text-muted mb-6">{t('auth.backToLogin')}</p>
  ...
  if (!token) {
    return (
      <div className="max-w-md mx-auto px-6 py-8 safe-area-top safe-area-bottom">
        <p className="text-red-600 mb-4">{t('errors.missingToken')}</p>
