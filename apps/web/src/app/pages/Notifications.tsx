  ...
  return (
    <div className="min-h-screen bg-surface-tint dark:bg-dark-bg max-w-md md:max-w-2xl mx-auto px-4 py-6 pb-24 md:pb-6">
      <header className="mb-6">
        <p className="text-xs text-primary-dark font-semibold uppercase tracking-wider mb-1">{t('notifications.updatesLabel')}</p>
        <h1 className="text-2xl font-semibold text-text-dark dark:text-white">{t('notifications.title')}</h1>
      </header>

      {loading && <p className="text-text-muted">{t('common.loading')}</p>}
      {error && (
        <ErrorState message={error} onRetry={fetchList} retryLabel={t('common.retry')} />
      )}
          ...
          {notifications.map((n) => (
            <li
              ...
              className={`rounded-2xl border border-input-border dark:border-dark-border overflow-hidden shadow-subtle ${
                n.read_at ? 'bg-surface dark:bg-dark-surface' : 'bg-primary/5 dark:bg-primary/10 border-primary/30'
              }`}
            >
              <div className="flex gap-3 p-4">
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-text-main dark:text-white">{notificationTitle(n)}</p>
                  {notificationBody(n) && (
                    <p className="text-sm text-text-muted dark:text-dark-text-secondary mt-0.5">{notificationBody(n)}</p>
                  )}
