...
export default function GroupDetail() {
  ...
  const tabs: { key: Tab; label: string; icon: string }[] = [
    ...
    { key: 'leaderboard', label: t('groups.leaderboard'), icon: 'emoji_events' },
    { key: 'members', label: t('groups.membersTab'), icon: 'group' },
  ];

  return (
    <div className="max-w-lg mx-auto pb-8 dark:bg-dark-bg min-h-screen">
      {/* Header */}
      <div className="px-4 pt-6">
        <button
          type="button"
          onClick={() => navigate('/groups')}
          className="flex items-center gap-2 text-text-muted dark:text-dark-text-secondary hover:text-primary mb-4"
        >
        ...
        <div className="flex items-start justify-between gap-4 mb-2">
          <div>
            ...
            <p className="text-text-muted dark:text-dark-text-secondary text-sm mt-0.5 flex items-center gap-1">
              <span className="material-symbols-outlined text-sm">person</span>
              {group.member_count ?? 0} {t('groups.members')}
            </p>
            {(group.start_date || group.end_date) && (
              <p className="text-xs text-text-muted dark:text-dark-text-secondary flex items-center gap-1 mt-1">
          ...
          <div>
            ...
            {!checklist || checklist.places.length === 0 ? (
              <div className="text-center py-10">
                ...
                {isAdmin && (
                  <p className="text-sm text-slate-400 mt-1">{t('groups.addPlacesToItinerary')}</p>
                )}
                {isAdmin && (
                  <button
                    type="button"
              ...
              <ol className="space-y-3">
                {checklist.places.map((place, idx) => (
                  <li
                    ...
                    className="rounded-2xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-surface overflow-hidden"
                  >
                    <button
                      type="button"
                    ...
                    >
                      <div className="flex-shrink-0 flex flex-col items-center gap-1">
                        <span className="text-xs font-bold text-slate-400 dark:text-dark-text-secondary">
                          {idx + 1}
                        </span>
                        <div
                          className={cn(
                          ...
                          <div>
                            ...
                            <div className="space-y-1">
                              {place.checked_in_by.map((ci) => (
                                <div key={ci.user_code} className="flex items-center gap-2 text-sm">
              ...
              <div className="flex justify-center items-end gap-3 mb-8 pt-6 px-2">
                {leaderboard[1] && (
                  <div className="flex flex-col items-center flex-1">
                    <div className="w-14 h-14 rounded-full border-2 border-slate-200 bg-white dark:bg-dark-surface flex items-center justify-center text-slate-700 dark:text-white font-bold text-lg mb-2">
                    ...
                    <div className="mt-3 h-16 w-full rounded-t-xl bg-slate-100 dark:bg-dark-border" />
                  </div>
                )}
                {leaderboard[0] && (
                  <div className="flex flex-col items-center flex-1 z-10">
                    <div className="w-20 h-20 rounded-full border-[3px] border-amber-400 bg-amber-50 dark:bg-amber-900/30 flex items-center justify-center text-amber-800 dark:text-amber-300 font-black text-2xl mb-2">
                      {leaderboard[0].display_name.charAt(0)}
                    </div>
                    <p className="text-sm font-black text-slate-800 dark:text-white truncate w-full text-center">
                    ...
                    <div className="mt-3 h-24 w-full rounded-t-xl bg-amber-100 dark:bg-amber-900/20" />
                  </div>
                )}
                {leaderboard[2] && (
                  <div className="flex flex-col items-center flex-1">
                    <div className="w-14 h-14 rounded-full border-2 border-orange-200 bg-orange-50 dark:bg-orange-900/20 flex items-center justify-center text-orange-700 dark:text-orange-300 font-bold text-lg mb-2">
                    ...
                    <div className="mt-3 h-12 w-full rounded-t-xl bg-orange-100 dark:bg-orange-900/20" />
                  </div>
                )}
              </div>
            )}
