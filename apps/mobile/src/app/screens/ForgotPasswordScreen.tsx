...
export default function ForgotPasswordScreen() {
  ...
  if (sent) {
    return (
      <View style={[styles.successContainer, { paddingTop: insets.top + 24, paddingBottom: insets.bottom + 24 }]}>
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      >
        ...
        <TouchableOpacity style={[styles.primaryButton, loading && styles.buttonDisabled]} onPress={handleSubmit} disabled={loading} activeOpacity={0.85}>
          <Text style={styles.primaryButtonText}>{loading ? t('common.loading') : t('auth.sendResetLink')}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  successContainer: { flex: 1, backgroundColor: tokens.colors.backgroundLight, paddingHorizontal: 24, alignItems: 'center', justifyContent: 'center', gap: 12 },
  successIconContainer: { width: 64, height: 64, borderRadius: 32, backgroundColor: '#D1FAE5', alignItems: 'center', justifyContent: 'center', marginBottom: 8 },
  ...
  keyIconContainer: { width: 56, height: 56, borderRadius: 28, backgroundColor: `${tokens.colors.primary}1A`, alignItems: 'center', justifyContent: 'center', marginBottom: 20 },
