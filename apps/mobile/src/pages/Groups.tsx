  ...
  return (
    <div className="max-w-md mx-auto px-5 py-6">
      <header className="mb-6 flex items-center justify-between">
        ...
        <Link to="/groups/new" className="flex items-center justify-center w-12 h-12 rounded-full bg-primary text-white shadow-lg" aria-label="Create group">
      ...
      {!loading && !error && groups.length === 0 && (
        <div className="text-center py-12 bg-gray-50 rounded-2xl border border-gray-100">
          ...
          <Link to="/groups/new" className="inline-block py-2 px-4 rounded-xl bg-primary text-white text-sm font-medium">Create a group</Link>
        </div>
      )}
      ...
      {!loading && !error && groups.length > 0 && (
        <div className="space-y-3">
          {groups.map((g) => (
            <Link key={g.group_code} to={`/groups/${g.group_code}`} className="block p-4 bg-white rounded-2xl border border-gray-200 hover:shadow-md transition-shadow">
