...
export default function GroupDetail() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    Promise.all([getGroup(groupCode), getGroupLeaderboard(groupCode), getGroupActivity(groupCode, 15)])
      .then(([g, lb, act]) => {
        if (!cancelled) {
          setGroup(g);
          setLeaderboard(lb);
  ...
  return (
    <div className="max-w-md mx-auto px-5 py-6 pb-24">
      <div className="flex items-center gap-3 mb-6">
        ...
        <button type="button" onClick={copyInviteLink} className="py-2 px-4 rounded-xl bg-primary text-white text-sm font-medium">Invite</button>
      </div>
