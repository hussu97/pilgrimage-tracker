...
def _iter_scraped_rows(scraper_engine, run_code: str | None, failed_only: bool):
    ...
    query = text(
        "SELECT place_code, name, raw_data, quality_score FROM scrapedplace"
...
def sync_places_for_run(
    *,
    ...
    def flush_chunk() -> None:
        ...
        _mark_status(scraper_engine, run_code, "synced", synced_codes)
        _mark_status(scraper_engine, run_code, "failed", failed_codes)
        chunk = []
        chunk_codes = []
    ...
    for row in _iter_scraped_rows(scraper_engine, run_code, failed_only):
        ...
        if quality_score is not None and float(quality_score) < _QUALITY_GATE:
            ...
            _mark_status(scraper_engine, run_code, "quality_filtered", [place_code])
            continue
        if not _is_name_specific_enough(name or ""):
            summary.skipped_name += 1
            _mark_status(scraper_engine, run_code, "name_filtered", [place_code])
            continue
        ...
        if place is None:
            ...
            _mark_status(scraper_engine, run_code, "failed", [place_code])
            continue
