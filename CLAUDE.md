...
All datetime model fields **must** use the `_UTCAwareDateTime` TypeDecorator defined in `server/app/db/models.py` via the `_TSTZ()` helper, **not** plain `DateTime` or `Field(default_factory=...)` alone.
