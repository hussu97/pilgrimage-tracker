...
All previously completed items from the original roadmap, preserved for reference.

### P0 — Critical Fixes (All Complete)

- [x] **Replace all `datetime.utcnow()` calls with `datetime.now(UTC)`**
  - `datetime.utcnow()` is deprecated in Python 3.12 and removed in Python 3.14.

- [x] **Fix N+1 query patterns in group listing**
  - `list_groups` was making 3 separate DB calls per group (members, last activity, progress).

- [x] **Fix N+1 query pattern in favorites endpoint**
  - `get_my_favorites` was fetching each place and its images individually in a loop.

- [x] **Fix N+1 query pattern in place reviews**
  - `get_place_reviews` was calling `get_review_images` per review in a loop.

- [x] **Implement place delete endpoint**
  - Was returning 501 Not Implemented. Removed entirely as it was unused.

### P1 — High Priority Improvements (All Complete)

- [x] **Replace `print()` with structured logging** (backend)
...
- [x] **Integrate error tracking service** (GlitchTip)

- [x] **Add network error feedback on mobile**

### P2 — Feature Enhancements (Completed Items)

- [x] **Place photo gallery** — horizontal scrollable gallery with full-screen viewer

- [x] **Search history and suggestions** — last 10 queries in local storage

- [x] **Multi-unit distance display** — km/mi based on user preference

- [x] **Social sharing with rich previews** — Open Graph meta tags, formatted share messages

- [x] **Place search autocomplete** — trigram-based search with debounced requests

- [x] **Success feedback for user actions** — animated popup with haptic feedback

- [x] **Admin panel and dashboard** (Phases 1-5) — foundation, entity CRUD, scraper management, content & config, dashboard & analytics

### P3 — Scalability & DevOps (Completed Items)

- [x] **Monitoring and structured logging** — `structlog` + JSON logging + GlitchTip integration

- [x] **API response compression** — GZipMiddleware enabled

- [x] **API versioning strategy** — URL-based versioning documented

- [x] **Use enums for magic string literals** — `StrEnum` types for status strings
